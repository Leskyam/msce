<%@ Page language="c#" Codebehind="mencion.aspx.cs" AutoEventWireup="True" Inherits="ISPCFG.Msce.clswfMencion" %>
...
<html>
	<head>
		<title>Maestría: Menciones</title>
	...
	<body leftmargin="0" topmargin="0" marginheight="0" marginwidth="0"><!-- PAGE TABLE; BEGIN -->
							...
							<tr valign="top">
								...
								<td> <!-- MAIN WINDOW; BEGIN -->
									<table id="Table4" cellspacing="0" cellpadding="0" width="545" border="0">
										<tr valign="top">
											<td id="td_bodyHeader" style="PADDING-RIGHT: 30px; PADDING-LEFT: 30px" valign="middle" align="center" width="545" height="65"><span class="subtit_A" id="spHeaderContent">MENCIONES</span> </td></tr>
										<tr valign="top">
											<td>
												<table id="Table5" cellspacing="0" cellpadding="15" width="100%" border="0">
													<tr>
														<td id="td_body_bg" valign="top" align="left" height="320">
															...
															<table id="tbl_Edit" width="80%" align="center" border="0">
																<!-- Nombre -->
																<tr valign="bottom">
		...
		*/
		...
		function Editar(Id)
		{
			...
			document.getElementById('trMencion'+Id).style.display='none';
		...
		function getAllMenciones()
		{
			// Obtener los datos de los módulos.
			var dvMenciones = ISPCFG.Msce.clswfMencion.getAllMenciones().value;
			// Obtener la referencia al elemento DIV que contendrá los datos de los módulos.
			var divListaMenciones = document.getElementById('dvListaMenciones')
			// Comprobar que no sea cero (0) la cantidad de registros (módulos).
			if(dvMenciones.Rows.length==0)
			{
				// Mostrar mensaje al usuario indicando que no existen módulos registrados.
				divListaMenciones.innerHTML = "<b>NO EXISTEN MENCIONES REGISTRADAS.</b>";
				Nuevo();
				dvMenciones = null;
				return;
			}
			...
			var strHTML_Body = '';
			for(var i=0; i<=(dvMenciones.Rows.length-1); ++i)
			{
				var Id = dvMenciones.Rows[i].Id;
				var Nombre = dvMenciones.Rows[i].Nombre;
				// Formar el cuerpo del registro actual.
				strHTML_Body += "\t<!-- TABLA PARA DAR FORMATO A UN REGISTRO -->\n";
				strHTML_Body += "\t\t<tr id=\"trMencion" + Id + "\" valign=\"top\">\n";
			...
			strHTML_Foot = "</table>\n";
			divListaMenciones.innerHTML = strHTML_Header + strHTML_Body + strHTML_Foot;
			document.getElementById('spEstadoActualizacion').innerHTML = '';
			setButtomsVisibility();
			dvMenciones=null; 
		}
		...
		*/
		function stopEditing()
		{
			...
			if(strMode=='Editar')
			{
				...
				if(document.getElementById('trMencion'+IdEditado)!=null)
				{
					document.getElementById('trMencion'+IdEditado).style.display='';
				}
			}
		...
		function Actualizar()
		{
			...
			var Mencion = ISPCFG.Msce.clswfMencion.getMencionClass().value;
			if(Mencion==null)
			{
				strErrorMessage = "Estimado usuario, ha sucedido un error que ha impedido actualizar los datos de la mención. Le sugerimos que lo intente nuevamente. Si el problema persiste, consulte con los administradores del sistema.";
				ProcessPageError(document.getElementById('tr_Notify'), document.getElementById('sp_Notify'), strErrorMessage);
				return;
			}
			// Obtener los valores de los campos.
			var textNombre = document.getElementById('txtNombre').value;
			var boResult = false;
			
			Mencion.Nombre = textNombre;
			
			if(strMode=='Nuevo')
			{
				//Modulo.Id = null; Esto no es necesario, la clase clsModulo lo hace sola.
				document.getElementById('spEstadoActualizacion').innerHTML = 'Agregando mención...';
				ISPCFG.Msce.clswfMencion.Agregar(Mencion, Actualizar_CallBack);
				Mencion = null;
				return;
			}
			if(strMode=='Editar')
			{
				Mencion.Id = IdEditado;
				document.getElementById('spEstadoActualizacion').innerHTML = 'Actualizando mención...';
				ISPCFG.Msce.clswfMencion.Actualizar(Mencion, Actualizar_CallBack);
				Mencion = null;
			}
		...
		function Actualizar_CallBack(res)
		{
			...
			if(res.error!=null)
			{
				if(strMode=='Nuevo')
				{
					strErrorMessage = "Estimado usuario, ha sucedido un error que ha impedido agregar la mención. Le sugerimos que lo intente nuevamente. Si el problema persiste, consulte con los administradores del sistema.";					
				}
				else
				{
					strErrorMessage = "Estimado usuario, ha sucedido un error que ha impedido actualizar los datos de la mención. Le sugerimos que lo intente nuevamente. Si el problema persiste, consulte con los administradores del sistema.";				
			...
			else
			{
				...
				{
					var strWarningMessage = "Estimado usuario, no se ha podido actualizar los datos de la mención, sin embargo no se ha reportado ningún error. Le sugerimos que lo intente nuevamente. Si el problema persiste, consulte con los administradores del sistema.";
		...
		function Eliminar(Id)
		{
			...
			var res = ISPCFG.Msce.clswfMencion.canBeDeleted(Id);
			if(res.value==null)
			{
				var strErrorMessage = "No se pudo comprobar si esta mención puede ser eliminada.";
				ProcessPageError(document.getElementById('tr_Notify'), document.getElementById('sp_Notify'), strErrorMessage);
				res = null;
				return;
			}
			if(res.value==false)
			{
				var strWarning = ISPCFG.Msce.clswfMencion.WhyCanNotBeDeleted(Id).value;
				if(strWarning!=null)
				{
					...
					for(x=0; x<=(strWarning.length-1); x++)
					{
						...
						ProcessPageWarning(document.getElementById('tr_Notify'), document.getElementById('sp_Notify'), 'Esta mención no puede ser eliminada por la(s) siguiente causa.', strWarning[x], boOmitirCloseButton);
					}
					res = null;
					strWarning = null;
					return;
				...
				{
					var strErrorMessage = "No se han podido obtener las causas que impiden eliminar esta mención.";
					ProcessPageError(document.getElementById('tr_Notify'), document.getElementById('sp_Notify'), strErrorMessage);
				}				
			}
			else
			{
				if(!window.confirm("Se eliminarán los datos de la mención seleccionado.")){return;}
				ISPCFG.Msce.clswfMencion.Eliminar(Id, Eliminar_CallBack);
				res = null;
			}
		}
		...
		function Eliminar_CallBack(res)
		{
			if(res.error!=null)
			{
				var strErrorMessage = "Estimado usuario, ha sucedido un error que ha impedido eliminar la mención. Le sugerimos que lo intente nuevamente. Si el problema persiste, consulte con los administradores del sistema.";
				ProcessPageError(document.getElementById('tr_Notify'), document.getElementById('sp_Notify'), strErrorMessage);
				res = null;
				return;
			}
			if(res.value==false)
			{
				var strWarning = "Estimado usuario, no se ha podido eliminar la mención, sin embargo no se ha reportado ningún error. Le sugerimos que lo intente nuevamente. Si el problema persiste, consulte con los administradores del sistema.";
			...
			else
			{
				getAllMenciones();
				document.body.scrollTop = 0;
				document.getElementById('spEstadoActualizacion').innerHTML = 'Mención eliminada.';
			}
		}
