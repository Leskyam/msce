...
<%@ Page language="c#" Codebehind="titulo_academico.aspx.cs" AutoEventWireup="True" Inherits="ISPCFG.Msce.clswfTitulo_academico" %>
<!doctype html public "-//w3c//dtd html 4.0 transitional//en" >
<html>
  <head>
		<title>Maestría: Título académico</title>
	...
	<body leftmargin="0" topmargin="0" marginheight="0" marginwidth="0"><!-- PAGE TABLE; BEGIN -->
							...
							<tr valign="top">
								...
								<td> <!-- MAIN WINDOW; BEGIN -->
									<table id="Table4" cellspacing="0" cellpadding="0" width="545" border="0">
										<tr valign="top">
											<td id="td_bodyHeader" style="PADDING-RIGHT: 30px; PADDING-LEFT: 30px" valign="middle" align="center" width="545" height="65"><span class="subtit_A" id="spHeaderContent">TÍTULOS ACADÉMICOS</span></td></tr>
										<tr valign="top">
											<td>
															...
															<table id="tbl_List" width="80%" align="center" border="0">
																...
																<tr valign="top">
																	<td colspan="2">
																		...
																		<div id="dvListaTitulosAcademicos">
		...
		*/
		...
		function Editar(Id)
		{
			...
			document.getElementById('trTituloAcademico'+Id).style.display='none';
			IdEditado = Id;
		...
		function getAllTitulosAcademicos()
		{
			// Obtener los datos de los módulos.
			var dvTitulosAcademicos = ISPCFG.Msce.clswfTitulo_academico.getAllTitulosAcademicos().value;
			// Obtener la referencia al elemento DIV que contendrá los datos de los módulos.
			var divListaTitulosAcademicos = document.getElementById('dvListaTitulosAcademicos')
			// Comprobar que no sea cero (0) la cantidad de registros (módulos).
			if(dvTitulosAcademicos.Rows.length==0)
			{
				// Mostrar mensaje al usuario indicando que no existen módulos registrados.
				divListaTitulosAcademicos.innerHTML = "<b>NO EXISTEN TÍTULOS ACADÉMICOS REGISTRADOS.</b>";
				Nuevo();
				dvTitulosAcademicos = null;
				return;
			}
			else
			{
			...
			var strHTML_Body = '';
			for(var i=0; i<=(dvTitulosAcademicos.Rows.length-1); ++i)
			{
				var Id = dvTitulosAcademicos.Rows[i].Id;
				var Nombre = dvTitulosAcademicos.Rows[i].Nombre;
				// Formar el cuerpo del registro actual.
				//strHTML_Body += "\t<!-- TABLA PARA DAR FORMATO A UN REGISTRO -->\n";
				strHTML_Body += "\t\t<tr id=\"trTituloAcademico" + Id + "\" valign=\"top\">\n";
			...
			strHTML_Foot = "</table>\n";
			divListaTitulosAcademicos.innerHTML = strHTML_Header + strHTML_Body + strHTML_Foot;
			document.getElementById('spEstadoActualizacion').innerHTML = '';
			setButtomsVisibility();
			dvTitulosAcademicos=null; 
		}
		...
		*/
		function stopEditing()
		{
			...
			if(strMode=='Editar')
			{
				...
				if(document.getElementById('trTituloAcademico'+IdEditado)!=null)
				{
					document.getElementById('trTituloAcademico'+IdEditado).style.display='';
				}
			}
		...
		function Actualizar()
		{
			...
			var TituloAcademico = ISPCFG.Msce.clswfTitulo_academico.getTituloAcademicoClass().value;
			if(TituloAcademico==null)
			{
				strErrorMessage = "Estimado usuario, ha sucedido un error que ha impedido actualizar los datos del título académico. Le sugerimos que lo intente nuevamente. Si el problema persiste, consulte con los administradores del sistema.";
				ProcessPageError(document.getElementById('tr_Notify'), document.getElementById('sp_Notify'), strErrorMessage);
				return;
			}
			// Obtener los valores de los campos.
			TituloAcademico.Nombre = document.getElementById('txtNombre').value;
			
			if(strMode=='Nuevo')
			{
				document.getElementById('spEstadoActualizacion').innerHTML = 'Agregando título académico...';
				ISPCFG.Msce.clswfTitulo_academico.Agregar(TituloAcademico, Actualizar_CallBack);
				TituloAcademico = null;
				return;
			}
			if(strMode=='Editar')
			{
				TituloAcademico.Id = IdEditado;
				document.getElementById('spEstadoActualizacion').innerHTML = 'Actualizando título académico...';
				ISPCFG.Msce.clswfTitulo_academico.Actualizar(TituloAcademico, Actualizar_CallBack);
				TituloAcademico = null;
			}
		}
		...
		function Actualizar_CallBack(res)
		{
			...
			if(res.error!=null)
			{
				if(strMode=='Nuevo')
				{
					strErrorMessage = "Estimado usuario, ha sucedido un error que ha impedido agregar el título académico. Le sugerimos que lo intente nuevamente. Si el problema persiste, consulte con los administradores del sistema.";					
				}
				else
				{
					strErrorMessage = "Estimado usuario, ha sucedido un error que ha impedido actualizar los datos del título académico. Le sugerimos que lo intente nuevamente. Si el problema persiste, consulte con los administradores del sistema.";				
			...
			else
			{
				...
				{
					var strWarningMessage = "Estimado usuario, no se ha podido actualizar los datos del título académico, sin embargo no se ha reportado ningún error. Le sugerimos que lo intente nuevamente. Si el problema persiste, consulte con los administradores del sistema.";
					ProcessPageWarning(document.getElementById('tr_Notify'), document.getElementById('sp_Notify'), null, strWarningMessage);
				}
				else
				{
					stopEditing();
					getAllTitulosAcademicos();
					strMode = 'default';
					setButtomsVisibility();
		...
		function Eliminar(Id)
		{
			...
			var res = ISPCFG.Msce.clswfTitulo_academico.canBeDeleted(Id);
			if(res.value==null)
			{
				var strErrorMessage = "No se pudo comprobar si este título académico puede ser eliminado.";
				ProcessPageError(document.getElementById('tr_Notify'), document.getElementById('sp_Notify'), strErrorMessage);
				res = null;
				return;
			}
			if(res.value==false)
			{
				var strWarning = ISPCFG.Msce.clswfTitulo_academico.WhyCanNotBeDeleted(Id).value;
				if(strWarning!=null)
				{
					...
					for(x=0; x<=(strWarning.length-1); x++)
					{
						...
						ProcessPageWarning(document.getElementById('tr_Notify'), document.getElementById('sp_Notify'), 'Este título académico no puede ser eliminado por la(s) siguiente causa.', strWarning[x], boOmitirCloseButton);
					}
					res = null;
					strWarning = null;
					return;
				...
				{
					var strErrorMessage = "No se han podido obtener las causas que impiden eliminar este título académico.";
					ProcessPageError(document.getElementById('tr_Notify'), document.getElementById('sp_Notify'), strErrorMessage);
				}				
			}
			else
			{
				if(!window.confirm("Se eliminarán los datos del título académico seleccionado.")){return;}
				ISPCFG.Msce.clswfTitulo_academico.Eliminar(Id, Eliminar_CallBack);
				res = null;
			}
		}
		...
		function Eliminar_CallBack(res)
		{
			if(res.error!=null)
			{
				var strErrorMessage = "Estimado usuario, ha sucedido un error que ha impedido eliminar el título académico. Le sugerimos que lo intente nuevamente. Si el problema persiste, consulte con los administradores del sistema.";
				ProcessPageError(document.getElementById('tr_Notify'), document.getElementById('sp_Notify'), strErrorMessage);
				res = null;
				return;
			}
			if(res.value==false)
			{
				var strWarning = "Estimado usuario, no se ha podido eliminar el título académico, sin embargo no se ha reportado ningún error. Le sugerimos que lo intente nuevamente. Si el problema persiste, consulte con los administradores del sistema.";
				ProcessPageWarning(document.getElementById('tr_Notify'), document.getElementById('sp_Notify'), null);
				res = null;
				return;
			}
			else
			{
				getAllTitulosAcademicos();
				document.body.scrollTop = 0;
				document.getElementById('spEstadoActualizacion').innerHTML = 'Título académico eliminado.';
			}
		}
